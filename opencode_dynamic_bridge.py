...
Environment:
  OPENCODE_HOME   Directory containing current_model (default: ~/.opencode)
  OPENCODE_CMD    Binary to invoke (default: opencode on PATH)
...
OPENCODE_HOME = os.path.expanduser(os.environ.get("OPENCODE_HOME", "~/.opencode"))
MODEL_FILE = os.path.join(OPENCODE_HOME, "current_model")
DEFAULT_MODEL = "opencode/minimax-m2.5-free"
OPENCODE_CMD = os.environ.get("OPENCODE_CMD", "opencode")
BRIDGE_PORT = int(os.environ.get("OPENCODE_BRIDGE_PORT", "5557"))
...
def set_model(model: str) -> None:
    """Write model file (creates parent directory if needed)."""
    os.makedirs(os.path.dirname(MODEL_FILE), exist_ok=True)
    with open(MODEL_FILE, "w", encoding="utf-8") as f:
        f.write(model)


def ensure_default_model_file() -> None:
    """Seed default model only if file is missing or empty (do not overwrite user choice)."""
    if os.path.exists(MODEL_FILE):
        try:
            with open(MODEL_FILE, encoding="utf-8") as f:
                if f.read().strip():
...
def main():
    ensure_default_model_file()

    server = HTTPServer(("127.0.0.1", BRIDGE_PORT), Handler)

    print(f"Dynamic OpenCode Bridge running on http://127.0.0.1:{BRIDGE_PORT}")
    print(f"Model file: {MODEL_FILE}")
    print(f"Current model: {get_current_model()}")
    print("\nTo change model:")
    print(f'  echo "opencode/gpt-5-nano" > {MODEL_FILE}')
    print(f"\nTo check current model:")
    print(f"  curl http://127.0.0.1:{BRIDGE_PORT}/")
    print("\nAPI endpoint:")
    print(f"  POST http://127.0.0.1:{BRIDGE_PORT}/v1/chat/completions")
